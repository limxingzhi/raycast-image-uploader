...
- MIME types derived dynamically via `src/lib/mime.ts` (static maps + magic byte detection). No manual maps or external packages to maintain. Unknown types fall back to `application/octet-stream` / `"bin"`.
